...
int wakeupFlag = 0;
int resultFlag = 0;
int listening_duration = 5; // 监听5秒麦克风
...
static void demo_mic(const char* session_begin_params)
{
	...
	/* demo several seconds recording */
	while(i++ < listening_duration)
		sleep(1);
	errcode = sr_stop_listening(&iat);
	if (errcode) {
...
int main(int argc, char* argv[])
{
	...
	ros::Subscriber wakeUpSub = n.subscribe("voiceWakeUp", 1000, WakeUpCb);
	...
	const char* login_params = "appid = 1aa92486, work_dir = .";
	...
	*/
	...
	while(ros::ok())
	{
		...
		if(wakeupFlag)
		{
			printf("Demo the speech from microphone\n");
